...
% t = 2142, 3102:6:3198
vid = false;
if vid, aviobj = avifile('output.avi','fps',2.5); end
did = 1;
Times = unique(D(D(:,1)==did,2))';
for t = D(D(:,1)==did & D(:,3)==8,2)'%Times(1:end)%3102:6:3198%Times(100:5:length(Times))
...
if vid
    aviobj = close(aviobj);
    system(['/opt/local/bin/ffmpeg -i output.avi '...
